...
function Dashboard() {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        const source = data.containerId
          ...
          ? "host"
          : "unknown";
        console.log(`📦 WebSocket 수신 [${source}]`, data); // ✅ 로그 추가
      ...
      data: {
        ...
        datasets: [
          ...
          },
          {
            ...
            yAxisID: "y1",
            data: Array(30).fill(
              (usage.memoryUsedBytes / usage.memoryTotalBytes) * 100
            ),
            borderColor: "#36a2eb",
          },
          {
            ...
            yAxisID: "y1",
            data: Array(30).fill(
              (usage.diskUsedBytes / usage.diskTotalBytes) * 100
            ),
            borderColor: "#ffce56",
          },
        ],
  ...
  const updateSummary = (data) => {
    ...
    const disk = data.diskTotalBytes
  ...
  const drawNetworkLive = (networkDelta) => {
    ...
    chart.data.datasets.forEach((dataset) => {
      ...
      const value =
        direction === "Recv"
          ? networkDelta[iface]?.rxBps ?? 0
          : networkDelta[iface]?.txBps ?? 0;
  ...
  const renderHostSelector = () => {
    ...
    hostData.forEach((h, i) => {
      ...
      btn.onclick = () => {
        document
          .querySelectorAll(".host-selector button")
  ...
  const drawNetwork = (host) => {
    ...
    const datasets = Object.entries(host.network).flatMap(([iface, val]) => [
      {
        label: `${iface} Recv`,
        data: Array(30).fill(Number(val.bytesReceived)),
        borderColor: "#4bc0c0",
      },
      {
        label: `${iface} Sent`,
        data: Array(30).fill(Number(val.bytesSent)),
        borderColor: "#9966ff",
      },
    ]);
    ...
    <div className={styles.dashboardWrapper}>
      ...
      <main className={styles.mainContent}>
        ...
        <div className={styles.thresholdSummary}>
          <p>
            ⚙️ 현재 임계치 - CPU: {thresholds.cpuPercent}%, Mem:{" "}
            {thresholds.memoryPercent}%, Disk: {thresholds.diskPercent}%
          </p>
        </div>
        {liveMetrics && (
          <div className={styles.liveMetricsBanner}>
            📈 실시간 메트릭 → CPU: {liveMetrics.cpuUsagePercent?.toFixed(1)}%,
            Mem:{" "}
            {(
              (liveMetrics.memoryUsedBytes / liveMetrics.memoryTotalBytes) *
              100
            ).toFixed(1)}
            %, Disk:{" "}
            {(
              (liveMetrics.diskUsedBytes / liveMetrics.diskTotalBytes) *
              100
            ).toFixed(1)}
            %
          </div>
              ...
              <div className={styles.cardHeader}>
                <div>
                  ...
                  <p className={styles.cardSubtitle}>
                    Hosts & Containers approaching resource limits
                  </p>
                </div>
